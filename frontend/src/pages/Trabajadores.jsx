...
import { HiOutlineDownload, HiOutlineUserAdd, HiOutlineFilter } from "react-icons/hi"; // Íconos
...
const Trabajadores = () => {
  ...
  useEffect(() => {
    const fetchTrabajadores = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/usuarios/listarUsuarios`, {
          method: "GET",
          headers: {
  ...
  const filteredTrabajadores = trabajadores.filter(
    (trabajador) =>
      `${trabajador.nombre} ${trabajador.apellido}`.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto bg-white shadow rounded-lg p-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-xl font-semibold text-gray-700">Trabajadores</h1>
          <div className="flex gap-4">
            ...
            {/* Botón Añadir Nuevo */}
            <button
              onClick={() => navigate("/add-trabajador")}
        ...
        {/* Mostrar la tabla o un mensaje de carga */}
        {loading ? (
          <div className="text-center text-gray-500">Cargando trabajadores...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse">
              ...
              <thead className="bg-gray-100 border-b">
                <tr>
                  <th className="py-3 px-6 text-left text-sm font-medium text-gray-600">Rol</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredTrabajadores.map((trabajador) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-6 text-sm text-gray-700">{trabajador.email}</td>
                    <td className="py-3 px-6 text-sm">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${
                          trabajador.estado ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}
                      >
